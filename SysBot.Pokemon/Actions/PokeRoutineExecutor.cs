...
namespace SysBot.Pokemon
{
    public abstract class PokeRoutineExecutor : SwitchRoutineExecutor<PokeBotConfig>
    {
        ...
        protected async Task EnterTradeCode(int code, CancellationToken token)
        {
            ...
            {
                int delay = Config.ConnectionType == PokeConnectionType.WiFi ? 0_500 : 0;
        ...
        {
            var data = Config.ConnectionType == PokeConnectionType.WiFi ? await Connection.ReadBytesAsync(InBattleRaidOffset, 1, token).ConfigureAwait(false) : ConnectionUSB.ReadBytes(InBattleRaidOffset, 1);
            return data[0] == 1;
        }
